...
from .common import positional_encodings_like, INF, EPSILON, TransformerEncoder, TransformerDecoder, PackedLSTM, \
    LSTMDecoderAttention, LSTMDecoder, Embedding, Feedforward, mask
...
class PointerGenerator(nn.Module):
    ...
    def forward(self, batch):
        ...
        context_encoded, (context_rnn_h, context_rnn_c) = self.bilstm_before_coattention(context_embedded,
                                                                                         context_lengths)
    ...
    def probs(self, generator, outputs, vocab_pointer_switches,
        ...
        p_context_ptr.scatter_add_(p_context_ptr.dim() - 1, context_indices.unsqueeze(1).expand_as(context_attention),
                                   context_attention)
...
class DualPtrRNNDecoder(nn.Module):
    ...
    def forward(self, input, context, output=None, hidden=None, context_alignment=None):
        ...
        context_outputs, vocab_pointer_switches, context_attention = [self.package_outputs(x) for x in
                                                                      [context_outputs, vocab_pointer_switches,
                                                                       context_attentions]]
