...
class DeepInteractiveEmbeddingNetworkV3(nn.Module):
    def __init__(self, field, args):
        ...
        self.equal = SumEqualCQBlock(args.dimension, args.transformer_heads,
                                     args.transformer_hidden, args.dropout_ratio)
    ...
    def forward(self, batch):
        ...
        context_encoded, question_encoded, (enc_h_context, enc_c_context), (
            enc_h_question, enc_c_question), context_refined, question_refined = self.equal(
            context_embedded, question_embedded,
        ...
        context_summary = torch.cat([coattended_context, context_hdd_scaled, context_refined], -1)

        condensed_context, _ = self.context_bilstm_after_coattention(context_summary, context_lengths)

        question_summary = torch.cat([coattended_question, question_hdd_scaled, question_refined], -1)
