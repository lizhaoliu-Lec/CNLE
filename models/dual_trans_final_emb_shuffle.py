...
from .common import EPSILON, PackedLSTM, LSTMDecoderAttention, LSTMDecoder, Feedforward, mask, DualTransformerEncoder, \
    FinalEmbedding


class DualTransNoSelfNetVFinalShuffle(nn.Module):
    ...
    def __init__(self, field, args):
        ...
        self.shuffle_rate = args.shuffle_rate

    def shuffle_question(self, question):

        if self.shuffle_rate > 0 and random.random() <= self.shuffle_rate:
            old_size = list(question.size())
            question_start = question[:, 0].unsqueeze(-1)
            question_end = question[:, -1].unsqueeze(-1)
            question_middle = question[:, 1:-1]
            if len(list(question_middle.size())) == 1:
                question_middle = question_middle.unsqueeze(-1)
            rand_idx = torch.randperm(question_middle.size()[-1])
            question = torch.cat([question_start, question_middle[:, rand_idx], question_end], dim=-1)

            new_size = list(question.size())
            assert old_size == new_size, 'question old size: %s, new size: %s' % (old_size, new_size)
        return question

    def shuffle_answer(self, answer, answer_lengths):

        if self.shuffle_rate > 0 and random.random() <= self.shuffle_rate:
            old_size = list(answer.size())
            min_answer_length = min(answer_lengths) - 2  # minus <init> and <eos>
            answer_start = answer[:, 0].unsqueeze(-1)
            answer_end = answer[:, min_answer_length + 1:]
            if len(list(answer_end.size())) == 1:
                answer_end = answer_end.unsqueeze(-1)
            answer_middle = answer[:, 1:min_answer_length + 1]
            if len(list(answer_middle.size())) == 1:
                answer_middle = answer_middle.unsqueeze(-1)
            rand_idx = torch.randperm(answer_middle.size(-1))
            answer = torch.cat([answer_start, answer_middle[:, rand_idx], answer_end], dim=-1)
            new_size = list(answer.size())
            assert old_size == new_size, 'answer old size: %s, new size: %s' % (old_size, new_size)
        return answer
    ...
    def forward(self, batch):
        ...
        if self.training:
            question, answer = self.shuffle_question(question), self.shuffle_answer(answer, answer_lengths)
