...
from .common import EPSILON, mask
from .common import PackedLSTM, LSTMDecoderAttention, LSTMDecoder
from .common import Feedforward, DualTransformerEncoder, FinalEmbedding


class DIENShare(nn.Module):
    ...
    def __init__(self, field, args):
        ...
        self.encoder_embeddings = FinalEmbedding(field, args.dimension,
                                                 ...
                                                 requires_grad=args.train_encoder_embedding)

        self.decoder_embeddings = FinalEmbedding(field, args.dimension,
                                                 dropout=args.dropout_ratio, project=True,
                                                 requires_grad=not args.freeze_decoder_embedding)
        ...
        self.bilstm_share_after = PackedLSTM(args.dimension, args.dimension,
                                             batch_first=True, dropout=dp(args), bidirectional=True,
                                             num_layers=args.rnn_layers)
    ...
    def forward(self, batch):
        ...
        final_context, (context_rnn_h, context_rnn_c) = self.bilstm_share_after(coattended_context[-1],
                                                                                context_lengths)
        context_rnn_state = [self.reshape_rnn_state(x) for x in (context_rnn_h, context_rnn_c)]

        final_question, _ = self.bilstm_share_after(coattended_question[-1], question_lengths)
        ...
        if self.training:
            ...
            decoder_outputs = self.dual_ptr_rnn_decoder(input=answer_embedded[:, :-1],
                                                        context=final_context,
                                                        question=final_question,
                                                        hidden=context_rnn_state)
            rnn_output, question_attention, context_question_switch, _ = decoder_outputs

            probs = self.probs(generator=self.out,
                               outputs=rnn_output,
                               vocab_question_switches=context_question_switch,
                               question_attention=question_attention,
                               question_indices=question_indices,
                               oov_to_limited_idx=oov_to_limited_idx)

            probs, targets = mask(answer_indices[:, 1:].contiguous(), probs.contiguous(), pad_idx=pad_idx)
            loss = F.nll_loss(probs.log(), targets)
            return loss, None
        else:
            return None, self.greedy(context=final_context,
                                     question=final_question,
                                     question_indices=question_indices,
                                     oov_to_limited_idx=oov_to_limited_idx,
                                     rnn_state=context_rnn_state).data
    ...
    def probs(self,
        ...
        p_vocab = F.softmax(scores, dim=scores.dim() - 1)  # B x L x V
        scaled_p_vocab = vocab_question_switches.expand_as(p_vocab) * p_vocab  # vocab_question_switches: B x L
        ...
        scaled_p_vocab.scatter_add_(scaled_p_vocab.dim() - 1,
                                    ...
                                    # question_indices: B x L -> B x L x L
                                    (1 - vocab_question_switches).expand_as(question_attention) * question_attention)

        # a bug in here #TODO
        # TODO to test
        return scaled_p_vocab.clamp(min=EPSILON)
    ...
    def greedy(self,
        ...
        for t in range(T):
            if t == 0:
                embedding = self.decoder_embeddings(context.new_full((B, 1),
            ...
            decoder_outputs = self.dual_ptr_rnn_decoder(input=hidden[:, t].unsqueeze(1),
                                                        context=context,
                                                        question=question,
                                                        hidden=rnn_state,
                                                        output=rnn_output)
            rnn_output, question_attention, context_question_switch, rnn_state = decoder_outputs
            probs = self.probs(generator=self.out,
                               outputs=rnn_output,
                               vocab_question_switches=context_question_switch,
                               question_attention=question_attention,
...
class DualPtrRNNDecoder(nn.Module):
    ...
    def __init__(self, d_in, d_hid, dropout=0.0, num_layers=1, input_feed=True):
        super().__init__()
        self.d_hid = d_hid
        self.d_in = d_in
        ...
        self.context_question_switch = nn.Sequential(Feedforward(2 * self.d_hid + d_in, 1),
                                                     nn.Sigmoid())

    def forward(self, input,
                context, question, output=None, hidden=None):
        context_output = output.squeeze(1) if output is not None else self.make_init_output(context)

        context_outputs, context_question_switches, question_attentions = [], [], []
        for emb_t in input.split(1, dim=1):
            ...
            context_output, _, _ = self.context_attn(dec_state, context)
            question_output, question_attention, _ = self.question_attn(dec_state, question)
        ...
        context_outputs, question_attention, context_question_switches = [
            self.package_outputs(x) for x in
            [context_outputs, question_attentions, context_question_switches]]
        return context_outputs, question_attention, context_question_switches, hidden
