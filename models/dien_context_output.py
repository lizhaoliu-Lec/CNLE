...
class DIENContext(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.training:
            ...
            probs = self.probs(generator=self.out,
                               outputs=rnn_output,
                               context_question_switches=context_question_switch,
    ...
    def probs(self,
        ...
        scaled_p_vocab.scatter_add_(scaled_p_vocab.dim() - 1,
                                    ...
                                    (1 - context_question_switches).expand_as(question_attention) * question_attention)
...
class DualPtrRNNDecoder(nn.Module):
    ...
    def __init__(self, d_in, d_hid, dropout=0.0, num_layers=1, input_feed=True):
        ...
        self.context_question_switch = nn.Sequential(Feedforward(3 * self.d_hid + d_in, 1),
                                                     nn.Sigmoid())
    ...
    def forward(self, input,
        ...
        for emb_t in input.split(1, dim=1):
            ...
            context_question_switch = self.context_question_switch(
                torch.cat([dec_state, context_output, question_output, emb_t], -1))
            context_output = self.dropout(context_output)
