...
class Simple(nn.Module):
    ...
    def forward(self, batch):
        ...
        coattended = self.coattention(context_embedded, question_embedded, context_padding,
                                                                   question_padding)
        coattened_context = [c[0] for c in coattended]
        coattened_question = [c[1] for c in coattended]
        final_context, (context_rnn_h, context_rnn_c) = self.bilstm_context(coattened_context[-1], context_lengths)
        context_rnn_state = [self.reshape_rnn_state(x) for x in (context_rnn_h, context_rnn_c)]

        final_question, (question_rnn_h, question_rnn_c) = self.bilstm_question(coattened_question[-1],
                                                                                question_lengths)
        ...
        if self.training:
            ...
            self_attended_decoded = self.self_attentive_decoder(answer_embedded[:, :-1].contiguous(),
                                                                coattened_context, context_padding=context_padding,
            ...
            rnn_output, rnn_state = decoder_outputs

            probs = self.probs(self.out, rnn_output,
            ...
            return None, self.greedy(coattened_context, final_context, final_question,
                                     context_indices, question_indices,
    ...
    def probs(self, generator, outputs,
              context_indices, question_indices,
              oov_to_limited_idx):
        # pdb.set_trace()
        ...
        if self.generative_vocab_size < effective_vocab_size:
            ...
            buff = p_vocab.new_full(size, EPSILON)
            p_vocab = torch.cat([p_vocab, buff], dim=buff.dim() - 1)
...
class DualPtrRNNDecoder(nn.Module):
    ...
    def forward(self, input, context, question, output=None, hidden=None):
        ...
        return self.package_outputs(context_outputs), hidden
