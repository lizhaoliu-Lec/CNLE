...
from .common import positional_encodings_like, INF, EPSILON, TransformerEncoder, TransformerDecoder, PackedLSTM, \
    LSTMDecoderAttention, LSTMDecoder, Embedding, Feedforward, mask, CoattentiveLayer, Refinement, \
    MutualAttentiveLayer


class DeepInteractiveEmbeddingNetworkV2(nn.Module):
    def __init__(self, field, args):
        ...
        self.bilstm_before_coattention_c = PackedLSTM(args.dimension, args.dimension,
                                                      batch_first=True, bidirectional=True, num_layers=1, dropout=0)

        self.bilstm_before_coattention_q = PackedLSTM(args.dimension, args.dimension,
                                                      batch_first=True, bidirectional=True, num_layers=1, dropout=0)

        # self.trans_before_encode_q = TransformerEncoder(args.dimension, args.transformer_heads,
        #                                                           args.transformer_hidden, args.transformer_layers,
        #                                                           args.dropout_ratio)
        # self.trans_after_encode_q = TransformerEncoder(args.dimension, args.transformer_heads,
        #                                                 args.transformer_hidden, args.transformer_layers,
        #                                                 args.dropout_ratio)

        # self.coattention = CoattentiveLayer(args.dimension, dropout=0.3)
        self.coattention = MutualAttentiveLayer(args.dimension, dropout=0.3)
        # self.mutualblock = MutualAttentiveBlock(args.dimension, dropout=0.3)
        # self.mutualblock2 = MutualAttentiveBlock(args.dimension, dropout=0.3)
        # self.mutualblock_2 = MutualAttentiveBlock(args.dimension, dropout=0.3)
        # dim = 2*args.dimension + args.dimension + args.dimension
        num_dim = 4
        dim = num_dim * args.dimension
        ...
        # TODO: Add Refinement layer
        self.refinement_context = Refinement(args.dimension, args.transformer_heads,
                                             args.transformer_hidden, args.transformer_layers,
                                             args.dropout_ratio)
        self.refinement_question = Refinement(args.dimension, args.transformer_heads,
                                              args.transformer_hidden, args.transformer_layers,
                                              args.dropout_ratio)

        # self.refinement_context_after_condense = Refinement(args.dimension, args.transformer_heads,
        #                                                     args.transformer_hidden, args.transformer_layers,
        #                                                     args.dropout_ratio)
        # self.refinement_question_after_condense = Refinement(args.dimension, args.transformer_heads,
        #                                                      args.transformer_hidden, args.transformer_layers,
        #                                                      args.dropout_ratio)

        # TODO: Final compression
        self.final_compress_context = PackedLSTM(2 * args.dimension, args.dimension,
                                                 batch_first=True, dropout=dp(args), bidirectional=True,
                                                 num_layers=args.rnn_layers)
        self.final_compress_question = PackedLSTM(2 * args.dimension, args.dimension,
                                                  batch_first=True, dropout=dp(args), bidirectional=True,
                                                  num_layers=args.rnn_layers)
    ...
    def forward(self, batch):
        ...
        if self.args.glove_and_char:
            # context_embedded: (b, seq_len_c, hidden)
            context_embedded = self.encoder_embeddings(context)
            # question_embedded: (b, seq_len_q, hidden)
        ...
        # Add transformer layer before and after question fed into LSTM
        # Question encoded by transformer
        # Fix bug but worse
        # question_trans_pre = self.trans_before_encode_q(question_embedded, padding=question_padding)
        # Bug but better
        # question_trans_pre = self.trans_before_encode_q(question_embedded, padding=question_padding)

        # TODO: Add Refinement layer before LSTM (DO NOT MODIFY)
        # question_refined: (b, seq_len_q, hidden)
        question_refined = self.refinement_question(question_embedded, context_embedded, padding=context_padding)
        # context_refined: (b, seq_len_c, hidden)
        context_refined = self.refinement_context(context_embedded, question_embedded, padding=question_padding)

        # context_encoded: (b, seq_len_c, hidden)
        # context_encoded = self.bilstm_before_coattention(context_embedded, context_lengths)[0]
        context_encoded, (enc_h_context, enc_c_context) = self.bilstm_before_coattention_c(context_embedded,
                                                                                           context_lengths)
        # question_encoded: (b, seq_len_q, hidden)
        # question_encoded = self.bilstm_before_coattention(question_embedded, question_lengths)[0]
        question_encoded, (enc_h_question, enc_c_question) = self.bilstm_before_coattention_q(question_embedded,
                                                                                              question_lengths)

        # enc_state_context: (b, hidden, 1)
        enc_state_context = [self.reshape_rnn_state(x) for x in (enc_h_context, enc_c_context)][0].permute(1, 2, 0)
        enc_state_question = [self.reshape_rnn_state(x) for x in (enc_h_question, enc_c_question)][0].permute(1, 2, 0)
        # context_state_attn: (b, seq_len_c, 1)
        # question_state_attn: (b, seq_len_q, 1)
        state_attn_c_T = 10
        state_attn_q_T = 2
        context_state_attn = F.softmax(context_encoded.bmm(enc_state_question) / state_attn_c_T, dim=1)
        question_state_attn = F.softmax(question_encoded.bmm(enc_state_context) / state_attn_q_T, dim=1)
        context_hdd_scaled = context_encoded * context_state_attn
        question_hdd_scaled = question_encoded * question_state_attn
        # context_encoded = context_encoded + context_hdd_scaled
        # question_encoded = question_encoded + question_hdd_scaled

        # TODO: Add Refinement layer after LSTM, replacing context_embedded with context_encoded
        # question_refined: (b, seq_len_q, hidden)
        # question_refined = self.refinement_question(question_encoded, context_encoded, padding=context_padding)
        # context_refined: (b, seq_len_c, hidden)
        # context_refined = self.refinement_context(context_encoded, question_encoded, padding=question_padding)
        coattended_context, coattended_question = self.coattention(context_encoded, question_encoded,
                                                                   context_padding, question_padding)

        # coattended_context2, coattended_question2, sum_of_question2, sum_of_context2 = self.mutualblock2(
        #     coattended_context1, coattended_question1,
        #     context_padding, question_padding,
        #     context_lengths, question_lengths)

        # coattended_context, coattended_question = self.mutualblock_2(coattended_context, coattended_question,
        #                                                            context_padding, question_padding)
        # coattended_context: (b, seq_len_c, 2 * hidden)
        # coattended_question: (b, seq_len_q, 2 * hidden)
        # coattended_context, coattended_question = self.coattention(context_encoded, question_encoded,
        #                                                            context_padding, question_padding)

        # Processing context
        # context_summary: (b, seq_len_c, 4 * hidden)
        # context_summary = torch.cat([coattended_context, context_encoded, context_embedded], -1)
        # context_summary = torch.cat([coattended_context, context_encoded, context_refined[-1]], -1)
        context_summary = torch.cat([coattended_context, context_hdd_scaled, context_refined[-1]], -1)
        # context_summary = torch.cat([coattended_context, sum_of_question, context_hdd_scaled, context_refined[-1]], -1)
        # context_summary = torch.cat([coattended_context, context_encoded, context_refined[-1], context_embedded], -1)
        # condensed_context: (b, seq_len_c, hidden)
        condensed_context, _ = self.context_bilstm_after_coattention(context_summary, context_lengths)

        # Processing question
        # question_summary: (b, seq_len_q, 4 * hidden)
        # question_summary = torch.cat([coattended_question, question_encoded, question_embedded], -1)
        # question_summary = torch.cat([coattended_question, question_encoded, question_refined[-1]], -1)
        question_summary = torch.cat([coattended_question, question_hdd_scaled, question_refined[-1]], -1)
        # question_summary = torch.cat([coattended_question, sum_of_context, question_hdd_scaled, question_refined[-1]], -1)
        # question_summary = torch.cat([coattended_question, question_encoded, question_refined[-1], question_embedded], -1)
        # condensed_question: (b, seq_len_q, hidden)
        condensed_question, _ = self.question_bilstm_after_coattention(question_summary, question_lengths)

        # TODO: Add refinement layer after condense
        # refined_condense_context = self.refinement_context_after_condense(condensed_context, condensed_question,
        #                                                                   padding=question_padding)
        # refined_condense_question = self.refinement_question_after_condense(condensed_question, condensed_context,
        #                                                                     padding=context_padding)

        # self_attended_context: [(b, seq_len_c, hidden), (b, seq_len_c, hidden), (b, seq_len_c, hidden)]
        # self_attended_context = self.self_attentive_encoder_context(refined_condense_context[-1],
        #                                                             padding=context_padding)
        # final_context: (b, seq_len_c, hidden)
        # context_rnn_h: (num_layers, b, hidden / 2), context_rnn_c: (num_layers, b, hidden / 2)
        final_context, (context_rnn_h, context_rnn_c) = self.bilstm_context(condensed_context, context_lengths)
        # TODO: Final compression with attended context and refined context
        # final_context, (context_rnn_h, context_rnn_c) = self.final_compress_context(
        #     torch.cat([self_attended_context[-1], context_refined[-1]], -1),
        #     context_lengths)
        # context_rnn_state: (1, b, hidden)
        context_rnn_state = [self.reshape_rnn_state(x) for x in (context_rnn_h, context_rnn_c)]

        # self_attended_question: [(b, seq_len_q, hidden), (b, seq_len_q, hidden), (b, seq_len_q, hidden)]
        # self_attended_question = self.self_attentive_encoder_question(refined_condense_question[-1],
        #                                                               padding=question_padding)
        # final_question: (b, seq_len_q, hidden)
        # context_rnn_h: (num_layers, b, hidden / 2), context_rnn_c: (num_layers, b, hidden / 2)
        final_question, (question_rnn_h, question_rnn_c) = self.bilstm_question(condensed_question,
                                                                                question_lengths)
        # final_question, (question_rnn_h, question_rnn_c) = self.final_compress_context(
        #     torch.cat([self_attended_question[-1], question_refined[-1]], -1),
        #     question_lengths)
        # pdb.set_trace()
        ...
        if self.training:
            ...
            # Set position encoding to false
            self_attended_decoded = self.self_attentive_decoder(answer_embedded[:, :-1].contiguous(),
                                                                [condensed_context], context_padding=context_padding,
            ...
            return None, self.greedy([condensed_context], final_context, final_question,
                                     context_indices, question_indices,
...
class DualPtrRNNDecoder(nn.Module):
    ...
    def forward(self, input, context, question, output=None, hidden=None, context_alignment=None,
        ...
        for emb_t in input.split(1, dim=1):
            ...
            # print("-----vocab_pointer_switch size-----\n", vocab_pointer_switch.size())
            # print("-----context_question_switch size-----\n", context_question_switch.size())
            # TODO: Only use question ptr
            # vocab_pointer_switch = torch.zeros([batch_size, 1], requires_grad=False).expand_as(vocab_pointer_switch).cuda()
            # context_question_switch = torch.zeros([batch_size, 1], requires_grad=False).expand_as(context_question_switch).cuda()
