...
from .common import positional_encodings_like, INF, EPSILON, TransformerEncoder, TransformerDecoder, PackedLSTM, \
    ...
    CoattentiveTransformerEncoderLayer

class InteractionNetworkV4(nn.Module):
    ...
    def __init__(self, field, args):
        ...
        # TODO maybe the first layer set dropout to 0
        self.equal_block1 = EqualContextAndQuestion(args.dimension, args.transformer_heads,
                                                    args.transformer_hidden, args.dropout_ratio)
        self.equal_block2 = EqualContextAndQuestion(args.dimension, args.transformer_heads,
                                                    args.transformer_hidden, args.dropout_ratio)
    ...
    def forward(self, batch):
        ...
        final_c, _, final_q, _ = self.equal_block1(context_embedded,
                                                   question_embedded,
                                                   context_lengths,
                                                   question_lengths,
                                                   context_padding,
        ...
        dual_c, final_c, (final_c_h, final_c_c), dual_q, final_q, _ = self.equal_block2(final_c,
                                                                                        final_q,
                                                                                        context_lengths,
                                                                                        question_lengths,
                                                                                        context_padding,
        ...
        context_rnn_state = [self.reshape_rnn_state(x) for x in (final_c_h, final_c_c)]
        ...
        if self.training:
            ...
            self_attended_decoded = self.self_attentive_decoder(answer_embedded[:, :-1].contiguous(),
                                                                [dual_c], context_padding=context_padding,
            ...
            decoder_outputs = self.dual_ptr_rnn_decoder(self_attended_decoded,
                                                        final_c, final_q, hidden=context_rnn_state)
            ...
            return None, self.greedy([dual_c], final_c, final_q,
                                     context_indices, question_indices,
