...
# for successfully download Glove and CharNGram
...
def initialize_logger(args, rank='main'):
    ...
    handler = handlers.RotatingFileHandler(os.path.join(args.log_dir, f'process_{rank}.log'), maxBytes=1024 * 1024 * 10,
                                           backupCount=1)
...
def prepare_data(args, field, logger):
    if field is None:
        ...
        FIELD = torchtext.data.ReversibleField(batch_first=True, init_token='<init>', eos_token='<eos>',
    ...
    if args.load is None:
        ...
        char_vectors = torchtext.vocab.CharNGram(cache=args.embeddings, unk_init=nn.init.kaiming_normal_)
        glove_vectors = torchtext.vocab.GloVe(cache=args.embeddings, unk_init=nn.init.kaiming_normal_)
...
def train(args, model, opt, train_iters, train_iterations, field, rank=0, world_size=1,
          log_every=10, val_every=100, save_every=1000, rounds=False, val_iters=[], writer=None, start_iteration=1,
          rnd=1):
                ...
                if not args.resume or iteration > start_iteration:
                    ...
                    if (val_every is not None and
                        ...
                        for val_task_idx, (val_task, val_iter) in enumerate(val_iters):
                            val_loss, metric_dict = validate(val_task, val_iter, model, logger, field, world_size, rank,
                                                             num_print=args.num_print, args=args)
                    ...
                    if save_every is not None and (iteration % args.save_every == 0 % args.save_every):
                        ...
                        if rank is not None and rank == 0:
                            torch.save({'model_state_dict': {k: v.cpu() for k, v in model.state_dict().items()},
                                        'field': field}, os.path.join(args.log_dir, f'iteration_{iteration}.pth'))
                        if world_size > 1:
                            torch.distributed.barrier()
                        torch.save(opt.state_dict(),
                                   os.path.join(args.log_dir, f'iteration_{iteration}_rank_{rank}_optim.pth'))
                        if world_size > 1:
                            torch.distributed.barrier()
                    ...
                    if args.warmup > 0 and args.transformer_lr:
                        ...
                        # param update
                    loss, train_metric_dict = step(model, batch, opt, iteration, field, task, lr=lr,
                    ...
                    if log_every is not None and (iteration % log_every == 0 % log_every):
                        ...
                        logger.info(
                            f'{args.timestamp}:{elapsed_time(logger)}:iteration_{iteration}:{round_progress}train_{task}:{task_progress}{avg_batch_size}loss_{local_loss:.4f}{metric_entry}')
                        num_examples = 0
                        len_contexts = 0
                        len_answers = 0
...
def run(args, run_args, rank=0, world_size=1):
    ...
    val_iters = [(name, to_iter(args, world_size, tok, x, device, train=False, token_testing=args.token_testing,
                                sort=False if 'sql' in name else None))
    ...
    if save_dict is not None:
        ...
        if args.resume:
            ...
            opt.load_state_dict(
                torch.load(os.path.join(args.save, f'{os.path.splitext(args.load)[0]}_rank_{rank}_optim.pth')))
...
def init_opt(args, model):
    ...
    if 'adam' in args.optimizer.lower():
        if args.transformer_lr:
            # opt = torch.optim.Adam(model.params, lr=1e-4, betas=(0.9, 0.98), eps=1e-9, weight_decay=1e-5)
            opt = torch.optim.Adam(model.params, lr=1e-4, betas=(0.9, 0.98), eps=1e-9)
        else:
            opt = torch.optim.Adam(model.params, lr=1e-4, betas=(args.beta0, 0.999))
            # opt = torch.optim.Adam(model.params, lr=1e-4, betas=(args.beta0, 0.999), weight_decay=1e-5)
    else:
