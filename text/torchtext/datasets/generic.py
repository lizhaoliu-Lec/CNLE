                        ...
                        for qa in qas:
                            ...
                            if len(qa['answers']) == 0:
                                ...
                                ex = data.Example.fromlist(
                                    [context, question, answer, CONTEXT_SPECIAL, QUESTION_SPECIAL, context_question],
                                    fields)
                                ...
                                ex = data.Example.fromlist(
                                    [tagged_context, question, answer, CONTEXT_SPECIAL, QUESTION_SPECIAL,
...
class WikiSQL(CQA, data.Dataset):
    ...
    def __init__(self, path, field, query_as_question=False, subsample=None, **kwargs):
        ...
        cache_name = os.path.join(os.path.dirname(path), '.cache',
        ...
        if os.path.exists(cache_name):
            ...
            with open(expanded_path) as example_file:
                for idx, line in enumerate(example_file):
                    ...
                    context_question = get_context_question(context, question)
                    ex = data.Example.fromlist(
                        [context, question, answer, CONTEXT_SPECIAL, QUESTION_SPECIAL, context_question, idx], fields)
                    examples.append(ex)
            ...
            with open(os.path.expanduser(path)) as f:
                for woz_id, line in enumerate(f):
                    ...
                    if example_dict['lang'] in description:
                        ...
                        ex = data.Example.fromlist(
                            [context, question, answer, CONTEXT_SPECIAL, QUESTION_SPECIAL, context_question, woz_id],
                            fields)
                        examples.append(ex)
...
class OntoNotesNER(CQA, data.Dataset):
    ...
    def __init__(self, path, field, one_answer=True, subsample=None,
                 path_to_files='.data/ontonotes-release-5.0/data/files', subtask='all', nones=True, **kwargs):
        fields = [(x, field) for x in self.fields]

        cache_name = os.path.join(os.path.dirname(path), '.cache', os.path.basename(path), str(subsample), subtask,
                                  str(nones))
        if os.path.exists(cache_name):
            print(f'Loading cached data from {cache_name}')
            examples = torch.load(cache_name)
                            ...
                            for line in lines:
                                ...
                                for entity_tuple in entities:
                                    ...
                                    if not context[start:end] == entity:
                                        ...
                                        break
                                if is_no_good:
                                    print('Throwing out example that looks poorly labeled: ', original.strip(), ' (',
                                          file_id.strip(), ')')
                                    continue
